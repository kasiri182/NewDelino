...
#define BSBackgroundColorAnimationDuration 1.0f
...
- (void)initialize
{
    ...
    self.leftActivationPercentage = 0.0f;
    self.rightActivationPercentage = 0.0f;
    self.defaultBackgroundColor = [UIColor clearColor];
    
    //NSLog(@"%f",CGRectGetWidth(self.frame));
    
    self.colorView = [[UIView alloc] initWithFrame:CGRectMake(10, 0, CGRectGetWidth(self.frame) - 20, CGRectGetHeight(self.frame)-10)];
    [[self.colorView layer] setCornerRadius:4.0f];
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    ...
    if (state == UIGestureRecognizerStateBegan || state == UIGestureRecognizerStateChanged)
    {
        ...
        percentage = fabs(percentage); // We are interested in absolute percentage and use the dragging direction from now on
    ...
    {
        CGFloat offset = self.contentView.frame.origin.x;
        CGFloat percentage = fabs([self percentageForOffset:offset]);
        ...
        if (self.draggingDirection == BSSwipeTableViewCellDirectionLeft && self.rightMode == BSSwipeTableViewCellModeExit && percentage > self.rightActivationPercentage)
        {
...
- (void)moveSideViewWithPercentage:(CGFloat)percentage
{
    ...
    position.y = (CGRectGetHeight(self.bounds) - CGRectGetHeight(self.sideView.bounds)) * 0.25f;
    ...
    {
        position.x = CGRectGetWidth(self.bounds)-20 - CGRectGetWidth(self.sideView.bounds);
    }
    else if (self.draggingDirection == BSSwipeTableViewCellDirectionLeft && offset >= CGRectGetWidth(self.sideView.bounds))
    {
        position.x = CGRectGetWidth(self.bounds)-20 - offset;
    }
...
{
    [UIView animateWithDuration:BSBounceAnimationDuration delay:0.0f usingSpringWithDamping:1 initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        CGRect frame = self.contentView.frame;
        frame.origin.x = 0.0f;
        self.contentView.frame = frame;
